...
#include "petface_over_person_roi_filterer_v2.hpp"
#include "roi_utils.hpp"
#include "logger/logger.hpp"

PetfaceOverPersonROIFiltererV2::PetfaceOverPersonROIFiltererV2(std::shared_ptr<ROIDataHandler> data_handler,std::vector<std::string> &_overlap_check_categories,std::vector<float> _thresholds)
    : IROIFilterer(data_handler){
        overlap_check_categories = _overlap_check_categories;
        thresholds = _thresholds;

}
PetfaceOverPersonROIFiltererV2::~PetfaceOverPersonROIFiltererV2(){

}

std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::filter(std::vector<vizpet::BoundingBox> roi_boxes, cv::Mat image){
    ...
    float small_criteria_multi = (max_object_count == 1) ? 0.1f : 0.3f;
        ...
        for(int i =0;i<roi_boxes.size();i++){
            //if(roi_boxes[i].tag_id == 5){
                LOGV("VZ Debug: before Filter tag id : [%d], tag class : [%s] confidence: [%f]",roi_boxes[i].tag_id,roi_boxes[i].tag.c_str(), roi_boxes[i].score);
            //}
        }
    #endif
    ...
    #endif
    //LOGV("VZ Debug: PetfaceOverPersonROIFiltererV2 Enablinng small box removal for flower and animal");
...
std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::remove_petface_over_person_boxes(std::vector<vizpet::BoundingBox> boxes){
...
std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::remove_pet_within_pet_boxes(std::vector<vizpet::BoundingBox> boxes){
    std::vector<vizpet::BoundingBox> filtered_boxes;
    for(int i=0;i<boxes.size();i++){
...
std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::ensure_petface_over_pet_boxes(std::vector<vizpet::BoundingBox> boxes){
    std::vector<vizpet::BoundingBox> filtered_boxes;
    // LOGV("VZ Debug: ensure_petface_over_pet_boxes No of boxes=[%d]",boxes.size());
    for(int j=0;j<boxes.size();j++){
        // LOGV("VZ Debug: ensure_petface_over_pet_boxes confidence=[%f] class_name=[%d]",boxes[j].score, boxes[j].tag_id);
        if(roi_data_handler->get_label(boxes[j].tag_id)!="pets"){
        ...
        if(roi_data_handler->get_label(boxes[j].tag_id) == "pets"){
            ...
            if(!added){
                for(int i = 0;i<boxes.size();i++){
                    if(roi_data_handler->get_label(boxes[i].tag_id) == "PetEyes"){
                        int overlap_pet = 0;
                        float overlap = ROIUtils::get_overlap_between_boxes(boxes[j].location,boxes[i].location);
                        #if DUMPBOX
                            LOGV("VZ Debug: ensure_petface_over_pet_boxes adding overlap=[%f]",overlap);
                        #endif
                        if(overlap > 0.8)
...
std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::remove_invalid_boxes(std::vector<vizpet::BoundingBox> boxes){
    std::vector<vizpet::BoundingBox> filtered_boxes;
    for(int i=0; i<boxes.size(); i++){
...
std::vector<vizpet::BoundingBox> PetfaceOverPersonROIFiltererV2::remove_multi_petface_with_in_pet_boxes(std::vector<vizpet::BoundingBox> boxes){
   #ifdef DUMPBOX
